...
```{r Processing, echo=TRUE, tidy=TRUE}
unzip("activity.zip")
...
```{r TimeSeries, echo=TRUE, tidy=TRUE}
...
There are a total of **`r sum(is.na(data))`** missing values (NA), all in the
_steps_ variable as shown in the summary bellow:

```{r NumberOfMV, echo=TRUE, tidy=TRUE}
summary(data)
```
...
```{r MissingValuesImputation, echo=TRUE, tidy=TRUE}
...
```{r HistogramAfterImputation, echo=TRUE, tidy=TRUE}
...
```{r addWeekdaysWeekends, echo=TRUE, tidy=TRUE}
...
```{r panelTimeSeries, echo=TRUE, tidy=TRUE}
library(lattice)
